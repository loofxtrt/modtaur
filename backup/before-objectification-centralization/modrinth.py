...
from .utils import Context, API_BASE, HEADERS, Project, Version, Dependency, File
from .parser import get_compatible_version, get_primary_jar
from .cache import get_cached_version_list, write_cache
from . import logger
...
def _load_depencencies(raw_deps: list[dict]) -> list[Dependency]:
    ...
    for raw in raw_deps:
        ...
        d = Dependency(
            #TEMP_slug=get_slug_from_id(project_id),
...
def resolve_dependencies(dependencies: list[Dependency], parent_slug: str, ctx: Context):
    ...
    logger.debug(parent_slug, title='resolve depencies')
    ...
    for d in dependencies:
        ...
        #TEMP# não prosseguir se o projeto já tiver sido visitado pelo software
        #TEMPif slug in ctx.resolved:
        #TEMP    continue
        #TEMPctx.resolved.add(slug)

        #TEMPproject = get_project(slug)
        project_id = d.project_id
        project = get_project(project_id)
        project_type = project.project_type

        resolve_project_downloading(project_id, project_type, ctx, is_dependency_for=parent_slug)
...
def get_slug_from_id(project_id: str):
    ...
    logger.debug(project_id, title='get slug from id')
...
def resolve_project_downloading(
    ...
    cache_version_dir.mkdir(exist_ok=True, parents=True)
