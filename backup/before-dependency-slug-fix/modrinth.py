...
def resolve_dependencies(dependencies: list[Dependency], parent_slug: str, ctx: Context):
    ...
    for d in dependencies:
        ...
        # não prosseguir se o projeto já tiver sido visitado pelo software
        if slug in ctx.resolved:
            continue
        ctx.resolved.add(slug)

        project = get_project(slug)
        project_type = project.project_type

        resolve_project_downloading(slug, project_type, ctx, is_dependency_for=parent_slug)
