...
def verify_compatiblity(modpack: str, version: str | None):
    ...
    for m in data.get('mods'):
        proj = get_project(m)
        #vers = get_version_list(m)

        #compatible = get_compatible_version(
        #    version_list=vers,
        #    project=proj,
        #    ctx=ctx
        #)
        ...
        if not compatible:
            logger.error(f'incompatível com a versão {version}', title=m)
        else:
            logger.success(f'compatível com a versão {version}', title=m)
