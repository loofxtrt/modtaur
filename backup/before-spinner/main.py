...
def get_compatible_version(project_data: dict, slug: str, ctx: Context) -> dict | None:
    ...
    if target is None:
        logger.error('alvo não encontrado, possivelmente por não ser compatível com o loader ou versão', slug)
        return
...
def resolve_project_downloading(
    ...
    if not response:
        logger.warning('pulando, erro ao obter os dados do projeto', slug)
        return
