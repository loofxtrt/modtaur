...
from .modrinth import resolve_project_downloading, get_project_data
from .parser import get_compatible_version
from .utils import read_json, DOTMINECRAFT, Context
from . import logger

def _context_from_modpack_data(modpack_data: dict) -> Context:
    return Context(
...
def verify_compatiblity(modpack: str):
    ...
    for m in data.get('mods'):
        proj = get_project_data(m, section=None)

        version = get_compatible_version(
            project_data=proj.get('project_type'),
            project_type='mod',
            slug=m
        )

        if not version:
            logger.error('inválido: ' + m)
        else:
            logger.success('válido: ' + m)
